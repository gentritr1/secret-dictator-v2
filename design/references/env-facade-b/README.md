# Half-collapsed facade references

Created for this project. Built-in image generation was requested as GPT Image 2; the tool did not return a model identifier, so that identifier is unverified. Every prompt is retained beside its output. Native image sizes: {"hero.png": [1354, 1161], "material-id.png": [1536, 1024], "layout-template.png": [1536, 1024], "sheet.png": [1536, 1024]}.
...
scripts/blender/open-rubble-facade-b.py opens an owned copy of the approved A source. scripts/blender/rubble-facade-b.py constructs the model through Blender MCP with baked mesh coordinates and preserved COL_wall/SOCKET_lamp. scripts/blender/export-rubble-facade-variant.py validates collections and exports only the six production nodes. Source audit, build transcript, fixed captures and runtime evidence are under design/reviews/rubble-phase-2/env-facade-b/. Repeated lamps are authored sockets but not logically claimed; a failed load uses the declared capsule fallback.
