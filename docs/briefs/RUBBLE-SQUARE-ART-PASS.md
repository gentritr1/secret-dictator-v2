...
1. **Sheet** with GPT Image 2: three true orthographic views (FRONT / SIDE / TOP), same metre scale, shared baseline, flat grey background, 1 m scale bar, exact counts of every part, role in the game, story wear, style, avoid list. Use the template in `design/concepts/rubble/README.md`.
...
**4. Citizens.** Eight silhouettes from the lineup frame: rubble woman, returning soldier (no insignia), black-marketeer, clerk, innkeeper, professor, nurse, teenager with cart. Keep the armature, clip names, `SOCKET_Label` and foot-centre root from `chr-citizen-base`; keep triangle counts within 1.6k each; painted cloth atlas; one accessory each readable at distance. The player uses the same variants.

**5. Lighting re-measure.** Re-run the warm classifier on the three states; the night trial must stay under the bible's 10% warm ceiling and the ground must not crush below the Gate 3 floor recorded in memory (`(1,11,20)` darkest trial pixel). Adjust albedo, not intensity, when the gap is albedo.
...
4. Warm% per state within the bible rule, measured with the same classifier as baseline; night-trial darkest pixel not below the Gate 3 floor.
