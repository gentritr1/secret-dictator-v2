 ...
 *
 * engine.js, ai.js and driver.js are byte-for-byte the same files Node runs, and
 * they are UMD: under Node they take the `module.exports` branch; in a browser
 * there is no `module`, so each one hangs itself off the global object instead
 * (`self.SD`, `self.SDAI`, `self.SDDriver`). They are not ESM and must not be
...
for (const [name, api, probe] of [
  ...
  if (!api || typeof api[probe] !== 'function') {
    throw new Error(
      ...
      'Check that src/engine/' + name.toLowerCase() + '.js ran and that the ' +
      'import order in src/engine/index.js is engine → ai → driver.'
    );
  }
}
