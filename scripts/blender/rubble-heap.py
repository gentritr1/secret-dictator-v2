...
assert ASSET in ['env-rubble-small','env-rubble-large','env-brick-stack']
...
for part in parts:
    ...
    for v in mesh.vertices:
        if v.co.x < -config['visualBounds'][0]/2+.004:v.co.x=-config['visualBounds'][0]/2
        if v.co.x > config['visualBounds'][0]/2-.004:v.co.x=config['visualBounds'][0]/2
    # Bevels stay inside each piece's envelope, with the sheet's peak restored.
    if max(z for x,z in outline)==config['visualBounds'][1]:
        for v in mesh.vertices:
    ...
    for face in mesh.polygons:
        ...
        for li in face.loop_indices:
            ...
            if ASSET=='env-brick-stack':
                ...
                uv.data[li].uv=(.02+(co[axes[0]]-minima[0])/spans[0]*.22,.88+(co[axes[1]]-minima[1])/spans[1]*.105)
            elif role=='Brick':
