...
with bpy.data.libraries.load(str(ROOT/'art/blender/environment/env-rubble-cart/env-rubble-cart.blend'),link=False) as (available,loaded):
    loaded.materials=[n for n in ['MAT_Metal'] if n not in bpy.data.materials]
...
for part in parts:
    ...
    for v in mesh.vertices:
        for axis,limit in [(0,config['visualBounds'][0]/2),(1,config['visualBounds'][2]/2)]:
            if v.co[axis]<-limit+.005:v.co[axis]=-limit
            if v.co[axis]>limit-.005:v.co[axis]=limit
        if v.co.z>config['visualBounds'][1]-.008:v.co.z=config['visualBounds'][1]
    ...
    for face in mesh.polygons:
        ...
        for li in face.loop_indices:
            ...
            else:uv.data[li].uv=(co[axes[0]]*.8+.3,co[axes[1]]*.8+.3)
...
camera.location=(2.2,-4.8,1.7);camera.rotation_euler=(Vector((0,0,config['visualBounds'][1]/2))-camera.location).to_track_quat('-Z','Y').to_euler();camera.data.lens=55;scene.camera=camera
