...
const out=process.env.WALK_OUTPUT || 'design/reviews/rubble-phase-2/env-facade-a';
...
try {
  ...
  const normalize=build=>({...build,colliders:build.colliders.map(c=>({...c,asset:/^env-facade-[abc]$/.test(c.asset)?'facade-grid-bay':c.asset}))});
  assert.deepEqual(normalize(report.builds.after),normalize(report.builds.before));
  report.comparison='Only facade asset IDs normalize to the same grid-bay role; all collider measurements, traces, anchors, sockets and tuning compare exactly.';
