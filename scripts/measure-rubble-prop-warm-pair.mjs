...
if(!asset || !out)throw Error('Usage: asset-id NEW-output-directory; no default output is allowed');
const config=JSON.parse(readFileSync(`design/references/${asset}/contract.json`));
const intendedCount=config.placements.length;
if(existsSync(out))throw Error('Output already exists; choose a new directory');
mkdirSync(out,{recursive:true});
const reference=`design/reviews/rubble-phase-2/${asset}/previous.glb`;
const oldBytes=readFileSync(reference);
const browser=await chromium.launch();
const report={method:'Same frozen scene, lights, camera and animation instant. Toggle all intended placements of the selected prop between the immutable 73a154c GLB and candidate; redraw shadows. Same rubble-pixels classifier, 1280x720, seed1000/7, podium camera. Three independent trial instants retained. Supplemental isolation measurement, not a replacement for full-scene captures.',allowancePp:0,errors:[],pairs:[]};
report.reference=reference;
report.resolutionPp=.001;
report.asset=asset;report.intendedPlacements=intendedCount;
report.method=report.method.replace('the immutable 73a154c GLB','the recorded previous dressing GLB');
try {
  const page=await browser.newPage({viewport:{width:1280,height:720},reducedMotion:'reduce'});
  page.on('pageerror',e=>report.errors.push(e.message));
  await page.route('**/__before-prop.glb',r=>r.fulfill({contentType:'model/gltf-binary',body:oldBytes}));
  ...
  await page.route('**/src/play/main.js*',async route=>{
    ...
    await route.fulfill({response,body:(await response.text()).replace('function frame(now) {','function frame(now) { if (window.__propStill) { requestAnimationFrame(frame); return; }')+'\nwindow.__propReview={scene,rig,renderer};\n'});
  });
  ...
  for (const [index,state] of ['day','dusk','trial','trial','trial'].entries()) {
    ...
    const count=await page.evaluate(async ({asset,collider})=>{
      window.__propStill=true;
      const A=await import('/src/play/assets.js');const {scene}=window.__propReview;
      const current=scene.children.filter(n=>n.name===asset);
      const template=await new A.GLTFLoader().loadAsync('/__before-prop.glb');
      const previous=A.ENVIRONMENT.filter(s=>s.id===asset).map(spec=>{
        const built=A.buildEnvironment(template.scene.clone(true),{...spec,requiredNodes:[collider]});
        if(!built.ok)throw Error(built.reason);built.visual.visible=false;scene.add(built.visual);return built.visual;
      });
      window.__propReview.current=current;window.__propReview.previous=previous;
      const camera=window.__propReview.rig.camera;
    ...
    },{asset,collider:config.collider});
    if(count.current!==intendedCount || count.previous!==intendedCount)throw Error('Expected all intended prop placements');
    await page.addScriptTag({content:'window.measurePropPixels='+measurePixels.toString()+';'});
    ...
    for(const variant of ['before','after']) {
      ...
      for(const hud of [true,false]) {
        ...
        const measured=await page.evaluate(async source=>{
          ...
          const ctx=c.getContext('2d');ctx.drawImage(im,0,0);const m=window.measurePropPixels(ctx.getImageData(0,0,1280,720).data,1280,720);
  ...
  if(report.errors.length || !report.passesZeroAllowance)throw Error('Prop paired warm gate failed');
